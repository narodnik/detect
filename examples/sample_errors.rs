...
fn main() {
    ...
    println!("ring                  : Z_{}[X]/(X^{} + 1)", param::Q, param::N_RING);
    println!("defining relation     : Phi_{}(X) = X^{} + 1", param::M, param::N_RING);
    ...
    println!("empirical std         : {std:.5}   (target {})", param::SIGMA);
    println!("empirical P(0)        : {:.5}", count(0) / m);
    println!("empirical P(+1) = P(-1): {:.5}", count(1) / m);
    println!("max |coeff|           : {maxabs}          (bound r = {})", param::R);
    ...
    println!("P(|noise| > r)        : {tail:.3e}   (need <= eps_n/ell = {:.3e})", param::EPS_N / param::ELL as f64);
